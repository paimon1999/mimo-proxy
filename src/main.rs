...
async fn main() {
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let after = guard.len();
            if before != after {
                info!("[CACHE] Cleaned {} expired entries, remaining {}", before - after, after);
            }
        }
    });
...
async fn proxy_handler(
    ...
    let cleaned = decoded
        ...
        .collect::<Vec<&str>>()
        .join("/");
    let cleaned_path = format!("/{}", cleaned);

    let upstream_path = if cleaned_path.starts_with("/v1/") {
        cleaned_path[3..].to_string() // 去掉 /v1 前缀，保留如 /chat/completions
    ...
    if let Some(v) = fwd.get("api-key").and_then(|v| v.to_str().ok()) {
        api_key = Some(v.to_string());
    }
    if api_key.is_none() {
        if let Some(v) = fwd.get("authorization").and_then(|v| v.to_str().ok()) {
    ...
    if let Some(k) = api_key {
        fwd.insert("api-key", HeaderValue::from_str(&k).unwrap());
    }
    ...
    if upstream_path == "/chat/completions" {
        if let Ok(mut json) = serde_json::from_slice::<JsonValue>(&body_bytes) {
            if let Some(arr) = json.get_mut("messages").and_then(|m| m.as_array_mut()) {
                ...
                if modified {
                    if hits > 0 {
                        info!("[PATCH] Injected {} reasoning_content from cache", hits);
                    } else {
                        info!("[PATCH] Injected empty reasoning_content for API compliance");
                    }
                    body_to_send = Bytes::from(serde_json::to_vec(&json).unwrap());
                }
            }
        }
    }
    ...
    if !body_to_send.is_empty() {
        ...
        info!("[REQUEST] Body: {}", &preview[..preview.len().min(800)]);
    }
    if cache_hits > 0 {
    ...
    if is_json && status.is_success() {
        if let Ok(json_body) = serde_json::from_slice::<JsonValue>(&bytes) {
            let cached = cache_response_body(&json_body, &state.cache, state.cache_ttl, state.cache_max_size).await;
            if cached > 0 {
    ...
    if !status.is_success() {
        warn!("[RESPONSE] Body: {}", &text[..text.len().min(1000)]);
    } else {
        info!("[RESPONSE] Body: {}", &text[..text.len().min(500)]);
    }
...
async fn fix_request_messages(
    ...
    for msg in messages.iter_mut() {
        ...
        if let Some(entry) = guard.get(&key) {
            if entry.expires_at > Instant::now() {
                ...
                if let Some(obj) = msg.as_object_mut() {
                    obj.insert("reasoning_content".into(), JsonValue::String(reasoning));
                    modified = true;
                    hits += 1;
                }
                continue;
        ...
        if let Some(obj) = msg.as_object_mut() {
            obj.insert("reasoning_content".into(), JsonValue::String(String::new()));
            modified = true;
        }
    }
